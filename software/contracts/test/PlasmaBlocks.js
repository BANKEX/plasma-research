...
contract('PlasmaBlocks', function ([_, wallet1, wallet2, wallet3, wallet4, wallet5]) {
  ...
  it('should revert on submit blocks with intersections', async function () {
    ...
    //
    await plasma.submitBlocks(0, '0x' + to160bits([1, 2, 3]));
    await plasma.submitBlocks(2, '0x' + to160bits([3, 4])).should.be.rejectedWith(EVMRevert);
    await plasma.submitBlocks(3, '0x' + to160bits([4, 5, 6]));
    await plasma.submitBlocks(6, '0x' + to160bits([]));
    await plasma.submitBlocks(3, '0x' + to160bits([4, 5, 6])).should.be.rejectedWith(EVMRevert);
    await plasma.submitBlocks(3, '0x' + to160bits([4, 5, 6])).should.be.rejectedWith(EVMRevert);
  ...
  it('should revert old blocks', async function () {
    ...
    await plasma.submitBlocks(1, '0x' + to160bits([2, 3])).should.be.rejectedWith(EVMRevert);
    await plasma.submitBlocks(1, '0x' + to160bits([8, 9])).should.be.rejectedWith(EVMRevert);
  });
  ...
  it('should deny blocks after gap', async function () {
    ...
    //
    await plasma.submitBlocks(0, '0x' + to160bits([1, 2, 3]));
    await plasma.submitBlocks(4, '0x' + to160bits([5, 6])).should.be.rejectedWith(EVMRevert);
  });
